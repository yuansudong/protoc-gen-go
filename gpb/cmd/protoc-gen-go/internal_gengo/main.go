...
import (
	...
	"github.com/yuansudong/protoc-gen-go/gpb/internal/encoding/tag"
...
// fieldXORMTagValue 生成xorm的tag,其目的是将结构体中的字段和数据库中的字段映射起来.
func fieldXORMTagValue(filed *protogen.Field) string {
	tag := ""
	switch filed.Desc.Kind() {
	case protoreflect.BoolKind:
	...
	case protoreflect.Uint32Kind:
		tag = "int unsigned not null default 0"
	case protoreflect.Int64Kind, protoreflect.Fixed64Kind, protoreflect.Sint64Kind, protoreflect.Sfixed64Kind:
		tag = "bigint not null default 0"
	case protoreflect.Uint64Kind:
		tag = "bigint unsigned not null default 0"
	case protoreflect.FloatKind:
		tag = "float(10,2) not null default 0"
	case protoreflect.DoubleKind:
		tag = "double(10,2) not null default 0"
	case protoreflect.StringKind:
		tag = "varchar(255) unsigned not null default ''"
	case protoreflect.BytesKind:
		tag = "blob"
	...
	if filed.Desc.IsList() || filed.Desc.IsMap() {
		// 进入这里,代表该类型,在数据库中需要是一个json
		// 该json被放在了一个Text的字段中.
		tag = "text json"
	}
	tag += " " + fmt.Sprintf("'%s'", string(filed.Desc.Name()))
	return tag
}
